"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const sakura = [
    {
        name: "Standing LP",
        outbreak: "4",
        persistence: "3",
        rigidity: "8",
        hit: "4",
        guard: "2",
        damage: "30",
        stan: "70",
        remarks: "Can be rapid canceled",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Standing MP",
        outbreak: "5",
        persistence: "3",
        rigidity: "13",
        hit: "7",
        guard: "3",
        damage: "60",
        stan: "100",
        remarks: "VS*Can only be canceled V-Skill II",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Standing HP",
        outbreak: "9",
        persistence: "3",
        rigidity: "21",
        hit: "3",
        guard: "-2",
        damage: "80",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Standing LK",
        outbreak: "4",
        persistence: "3",
        rigidity: "12",
        hit: "-1",
        guard: "-2",
        damage: "30",
        stan: "70",
        remarks: "",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Standing MK",
        outbreak: "8",
        persistence: "3",
        rigidity: "17(*15)",
        hit: "3",
        guard: "1",
        damage: "60",
        stan: "100",
        remarks: "VS*Can only be canceled V-Skill II\n*On whiff",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Standing HK",
        outbreak: "12",
        persistence: "2",
        rigidity: "26",
        hit: "1",
        guard: "-4",
        damage: "80",
        stan: "150",
        remarks: "Triggers Crush Counter  (+20F)\n                                                                                                                                                                VS*Can only be canceled V-Skill II",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching LP",
        outbreak: "3",
        persistence: "2",
        rigidity: "9",
        hit: "3",
        guard: "1",
        damage: "20",
        stan: "70",
        remarks: "Can be rapid canceled",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching MP",
        outbreak: "6",
        persistence: "4",
        rigidity: "13",
        hit: "5",
        guard: "2",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching HP",
        outbreak: "8",
        persistence: "5",
        rigidity: "23",
        hit: "-2",
        guard: "-5",
        damage: "90",
        stan: "150",
        remarks: "Forces stand",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching LK",
        outbreak: "4",
        persistence: "2",
        rigidity: "8",
        hit: "3",
        guard: "0",
        damage: "20",
        stan: "70",
        remarks: "VS*Can only be canceled V-Skill II",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching MK",
        outbreak: "6",
        persistence: "2",
        rigidity: "14",
        hit: "2",
        guard: "-3",
        damage: "50",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching HK",
        outbreak: "8",
        persistence: "2",
        rigidity: "25",
        hit: "D",
        guard: "-13",
        damage: "90",
        stan: "150",
        remarks: "Triggers Crush Counter  (D)\n                                                                                                                                                                VS*Can only be canceled V-Skill II",
        vtrigger: 1,
        type: "normal",
        command: ""
    },
    {
        name: "Jumping LP",
        outbreak: "4",
        persistence: "6",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "40",
        stan: "70",
        remarks: "",
        vtrigger: 1,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping MP",
        outbreak: "6",
        persistence: "7",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "60",
        stan: "100",
        remarks: "Causes blowback knockdown on airborne hit",
        vtrigger: 1,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping HP",
        outbreak: "8",
        persistence: "5",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "90",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping LK",
        outbreak: "4",
        persistence: "8",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "40",
        stan: "70",
        remarks: "Can cross-up",
        vtrigger: 1,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping MK",
        outbreak: "7",
        persistence: "5",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping HK",
        outbreak: "10",
        persistence: "5",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "90",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "jump",
        command: ""
    },
    {
        name: "Floral Kick",
        outbreak: "21",
        persistence: "2",
        rigidity: "23",
        hit: "1",
        guard: "-7",
        damage: "70",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "unique",
        command: "→ + MK"
    },
    {
        name: "Furiko Upper",
        outbreak: "7",
        persistence: "3",
        rigidity: "26",
        hit: "-4",
        guard: "-7",
        damage: "80",
        stan: "150",
        remarks: "Triggers Crush Counter  (D)",
        vtrigger: 1,
        type: "unique",
        command: "← + HP"
    },
    {
        name: "Chin Buster Kick",
        outbreak: "12",
        persistence: "3",
        rigidity: "20",
        hit: "4",
        guard: "2",
        damage: "80",
        stan: "150",
        remarks: "Forces stand\n                                                                                                                VS*Can only be canceled V-Skill II\nCauses blowback knockdown on airborne hit",
        vtrigger: 1,
        type: "unique",
        command: "↘ + HK"
    },
    {
        name: "Hanakaze",
        outbreak: "5",
        persistence: "3",
        rigidity: "17",
        hit: "",
        guard: "",
        damage: "130",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "throw",
        command: "(NEAR OPPONENT) → OR + LP LK"
    },
    {
        name: "Sakura Shoulder Throw",
        outbreak: "5",
        persistence: "3",
        rigidity: "17",
        hit: "",
        guard: "",
        damage: "150",
        stan: "200",
        remarks: "",
        vtrigger: 1,
        type: "throw",
        command: "(NEAR OPPONENT) ← + LP LK"
    },
    {
        name: "Sailor Hop",
        outbreak: "5",
        persistence: "2",
        rigidity: "6 frame(s) after landing",
        hit: "",
        guard: "",
        damage: "160",
        stan: "200",
        remarks: "",
        vtrigger: 1,
        type: "throw",
        command: "(CLOSE TO OPPONENT IN AIR) LP LK"
    },
    {
        name: "[VS1] Harukaze",
        outbreak: "",
        persistence: "",
        rigidity: "54+15 frame(s) after landing",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "",
        vtrigger: 1,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK"
    },
    {
        name: "[VS1] Sakura Otoshi",
        outbreak: "11",
        persistence: "3",
        rigidity: "6 frame(s) after landing(*115)",
        hit: "D",
        guard: "-2",
        damage: "50 (*2100) (*3150)",
        stan: "40 (*2120) (*3240)",
        remarks: "Can be performed up to 3 times on hit only. Startup for the 2nd and 3rd hits is 5F\nValue upon making contact with a standing Ryu, blocking, at the highest point\n+6F on block at the lowest point\n-9F on block at the highest point\nLanding recovery for the 3rd attack, hit only, is 5F\n*1on hit\n*2Up to the 2nd hit\n*3Up to the 3rd hit",
        vtrigger: 1,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK ▶ P"
    },
    {
        name: "[VS1] Oukakyaku",
        outbreak: "18",
        persistence: "Until landing",
        rigidity: "11 frame(s) after landing",
        hit: "3",
        guard: "-2",
        damage: "80",
        stan: "100",
        remarks: "Value upon making contact with a standing Ryu at the highest point\nLowest point: hit +12F, block +7F\nHighest point: hit +0F, block -5F",
        vtrigger: 1,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK ▶ K"
    },
    {
        name: "[VS1] Kashinfu",
        outbreak: "8",
        persistence: "3",
        rigidity: "15 frame(s) after landing",
        hit: "D",
        guard: "",
        damage: "150",
        stan: "200",
        remarks: "Can only throw standing opponents",
        vtrigger: 1,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK ▶ LP LK"
    },
    {
        name: "[VS2] Haru Hayate",
        outbreak: "",
        persistence: "",
        rigidity: "34 total frames",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "",
        vtrigger: 1,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL II) MP MK"
    },
    {
        name: "[VS2] Haru Hayate (Attack)",
        outbreak: "26",
        persistence: "5",
        rigidity: "14+11 frame(s) after landing",
        hit: "D",
        guard: "-7",
        damage: "70",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL II) MP MK (HOLD BUTTONS)"
    },
    {
        name: "Haruarashi",
        outbreak: "1",
        persistence: "",
        rigidity: "5",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "V-Gauge Timer + 3000F\n                                                                                                                                                                                While active, Hadoken, Tengyo Hadoken, and CA will be strengthened",
        vtrigger: 1,
        type: "vsystem",
        command: "HP HK"
    },
    {
        name: "Floral Spin",
        outbreak: "17",
        persistence: "2",
        rigidity: "24(*27)",
        hit: "D",
        guard: "-2",
        damage: "60",
        stan: "0",
        remarks: "1F - 30F  attack & projectile invincibility\n                                                Recover 200 stun upon activation\non hit",
        vtrigger: 1,
        type: "vsystem",
        command: "(DURING GUARD) → + LK"
    },
    {
        name: "V-Shift",
        outbreak: "1",
        persistence: "9",
        rigidity: "22",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "1F - 16F  throw invincibility\n                                                Allows you to evade strikes and projectiles from frames 1-9 (Increases the V-Gauge by 150 when the evasion triggers)\nCounterable during recovery\n17 frame move that's fully invincible until the end of recovery when evading attacks.\nTransitions to V-Shift Break when evading attacks.",
        vtrigger: 1,
        type: "vsystem",
        command: "HP MK"
    },
    {
        name: "Floral Side-Kick",
        outbreak: "8",
        persistence: "3",
        rigidity: "23",
        hit: "D",
        guard: "-2",
        damage: "60",
        stan: "0",
        remarks: "1F - 10F  full invincibility",
        vtrigger: 1,
        type: "vsystem",
        command: "HP MK ▶ (AFTER EVADING WITH V-SHIFT) HP MK"
    },
    {
        name: "L Hadoken",
        outbreak: "15",
        persistence: "42",
        rigidity: "48 total frames",
        hit: "-2",
        guard: "-8",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + LP"
    },
    {
        name: "M Hadoken",
        outbreak: "15",
        persistence: "49",
        rigidity: "48 total frames",
        hit: "-2",
        guard: "-8",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + MP"
    },
    {
        name: "H Hadoken",
        outbreak: "15",
        persistence: "56",
        rigidity: "48 total frames",
        hit: "-2",
        guard: "-8",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + HP"
    },
    {
        name: "Hadoken (Charge)",
        outbreak: "41",
        persistence: "",
        rigidity: "71 total frames",
        hit: "D",
        guard: "3",
        damage: "80",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + P (HOLD BUTTONS)"
    },
    {
        name: "VHadoken",
        outbreak: "14",
        persistence: "",
        rigidity: "42 total frames",
        hit: "D",
        guard: "1",
        damage: "80",
        stan: "150",
        remarks: "Uses 800F V-Gauge Timer\n                                                                                                                                                                                VS*Can be canceled into V-Skill II on hit or block only",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + P"
    },
    {
        name: "VHadoken (Charge)",
        outbreak: "33",
        persistence: "",
        rigidity: "60 total frames",
        hit: "D",
        guard: "10",
        damage: "100",
        stan: "150",
        remarks: "Uses 800F V-Gauge Timer",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + P (HOLD BUTTONS)"
    },
    {
        name: "EX Hadoken",
        outbreak: "14",
        persistence: "",
        rigidity: "45 total frames",
        hit: "D",
        guard: "1",
        damage: "100",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + P P"
    },
    {
        name: "Tengyo Hadoken",
        outbreak: "17",
        persistence: "27",
        rigidity: "45 total frames",
        hit: "D",
        guard: "4",
        damage: "60",
        stan: "100",
        remarks: "Will not hit crouching opponents",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + K"
    },
    {
        name: "Tengyo Hadoken (Charge)",
        outbreak: "39",
        persistence: "34",
        rigidity: "75 total frames",
        hit: "D",
        guard: "1",
        damage: "100",
        stan: "150",
        remarks: "Will not hit crouching opponents",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + K (HOLD BUTTONS)"
    },
    {
        name: "VTengyo Hadoken",
        outbreak: "15",
        persistence: "34",
        rigidity: "45 total frames",
        hit: "D",
        guard: "9",
        damage: "80",
        stan: "150",
        remarks: "Will not hit crouching opponents\n                                                                                                VS*Can be canceled into V-Skill II on hit or block only",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + K"
    },
    {
        name: "VTengyo Hadoken (Charge)",
        outbreak: "31",
        persistence: "34",
        rigidity: "55 total frames",
        hit: "D",
        guard: "13",
        damage: "110",
        stan: "150",
        remarks: "Will not hit crouching opponents",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + K (HOLD BUTTONS)"
    },
    {
        name: "EX Tengyo Hadoken",
        outbreak: "15",
        persistence: "34",
        rigidity: "45 total frames",
        hit: "D",
        guard: "-1",
        damage: "100",
        stan: "150",
        remarks: "Will not hit crouching opponents",
        vtrigger: 1,
        type: "special",
        command: "↓↘→ + K K"
    },
    {
        name: "L Shouoken",
        outbreak: "4",
        persistence: "7",
        rigidity: "26+18 frame(s) after landing",
        hit: "D",
        guard: "-31",
        damage: "100",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "→↓↘ + LP"
    },
    {
        name: "M Shouoken",
        outbreak: "9",
        persistence: "7",
        rigidity: "28+18 frame(s) after landing",
        hit: "D",
        guard: "-32",
        damage: "110",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "→↓↘ + MP"
    },
    {
        name: "H Shouoken",
        outbreak: "14",
        persistence: "12",
        rigidity: "34+18 frame(s) after landing",
        hit: "D",
        guard: "-37",
        damage: "120",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "→↓↘ + HP"
    },
    {
        name: "EX Shouoken",
        outbreak: "4",
        persistence: "18",
        rigidity: "36+21 frame(s) after landing",
        hit: "D",
        guard: "-45",
        damage: "150",
        stan: "200",
        remarks: "1F - 12F  full invincibility\n                                                Subject to counter hit during move duration (1.2x damage)",
        vtrigger: 1,
        type: "special",
        command: "→↓↘ P P"
    },
    {
        name: "L Shunpukyaku",
        outbreak: "17",
        persistence: "2",
        rigidity: "13+9 frame(s) after landing",
        hit: "2",
        guard: "-2",
        damage: "60",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↙← + LK"
    },
    {
        name: "M Shunpukyaku",
        outbreak: "21",
        persistence: "4",
        rigidity: "10+14 frame(s) after landing",
        hit: "3",
        guard: "-6",
        damage: "80",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↙← + MK"
    },
    {
        name: "H Shunpukyaku",
        outbreak: "24",
        persistence: "6",
        rigidity: "8+14 frame(s) after landing",
        hit: "2",
        guard: "-2",
        damage: "100",
        stan: "150",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "↓↙← + HK"
    },
    {
        name: "EX Shunpukyaku",
        outbreak: "15(*12)",
        persistence: "6",
        rigidity: "4+18 frame(s) after landing",
        hit: "",
        guard: "-2",
        damage: "",
        stan: "",
        remarks: "*When canceled by standing light punch, crouching light punch, crouching medium kick\nV*Third attack can be canceled when blocked",
        vtrigger: 1,
        type: "special",
        command: "↓↙← + K K"
    },
    {
        name: "EX Shunpukyaku (Hit)",
        outbreak: "15",
        persistence: "11",
        rigidity: "23",
        hit: "D",
        guard: "",
        damage: "120",
        stan: "150",
        remarks: "V*Only possible during 3rd attack",
        vtrigger: 1,
        type: "special",
        command: "↓↙← + K K"
    },
    {
        name: "Airborne Shunpukyaku",
        outbreak: "13",
        persistence: "4",
        rigidity: "16 frame(s) after landing",
        hit: "D",
        guard: "",
        damage: "100",
        stan: "150",
        remarks: "Strength changes travel distance",
        vtrigger: 1,
        type: "special",
        command: "(DURING JUMP) ↓↙← + K"
    },
    {
        name: "EX Airborne Shunpukyaku",
        outbreak: "12",
        persistence: "19",
        rigidity: "16 frame(s) after landing",
        hit: "D",
        guard: "",
        damage: "140",
        stan: "200",
        remarks: "",
        vtrigger: 1,
        type: "special",
        command: "(DURING JUMP) ↓↙← + K K"
    },
    {
        name: "Hogasho",
        outbreak: "21",
        persistence: "7",
        rigidity: "22",
        hit: "9",
        guard: "5",
        damage: "80",
        stan: "150",
        remarks: "Uses 500F V-Gauge Timer\n                                                                                                                                                                                Cannot be performed by canceling from a special move cancellable move\nNote: Recovery is extended by 5F on whiff",
        vtrigger: 1,
        type: "special",
        command: "(DURING V-TRIGGER I) HP HK"
    },
    {
        name: "Sakura Rain",
        outbreak: "1+5",
        persistence: "",
        rigidity: "77 total frames",
        hit: "D",
        guard: "-23",
        damage: "250（*330）",
        stan: "0",
        remarks: "1F - 7F  full invincibility\n                                                *During release hit",
        vtrigger: 1,
        type: "ca",
        command: "↓↘→ ↓↘→ + P"
    },
    {
        name: "VSakura Rain",
        outbreak: "1+5",
        persistence: "",
        rigidity: "77 total frames",
        hit: "D",
        guard: "-23",
        damage: "280（*350）",
        stan: "0",
        remarks: "1F - 7F  full invincibility\n                                                *During release hit",
        vtrigger: 1,
        type: "ca",
        command: "(DURING V-TRIGGER I) ↓↘→ ↓↘→ + P"
    },
    {
        name: "Standing LP",
        outbreak: "4",
        persistence: "3",
        rigidity: "8",
        hit: "4",
        guard: "2",
        damage: "30",
        stan: "70",
        remarks: "Can be rapid canceled",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Standing MP",
        outbreak: "5",
        persistence: "3",
        rigidity: "13",
        hit: "7",
        guard: "3",
        damage: "60",
        stan: "100",
        remarks: "VS*Can only be canceled V-Skill II",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Standing HP",
        outbreak: "9",
        persistence: "3",
        rigidity: "21",
        hit: "3",
        guard: "-2",
        damage: "80",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Standing LK",
        outbreak: "4",
        persistence: "3",
        rigidity: "12",
        hit: "-1",
        guard: "-2",
        damage: "30",
        stan: "70",
        remarks: "",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Standing MK",
        outbreak: "8",
        persistence: "3",
        rigidity: "17(*15)",
        hit: "3",
        guard: "1",
        damage: "60",
        stan: "100",
        remarks: "VS*Can only be canceled V-Skill II\n*On whiff",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Standing HK",
        outbreak: "12",
        persistence: "2",
        rigidity: "26",
        hit: "1",
        guard: "-4",
        damage: "80",
        stan: "150",
        remarks: "Triggers Crush Counter  (+20F)\n                                                                                                                                                                VS*Can only be canceled V-Skill II",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching LP",
        outbreak: "3",
        persistence: "2",
        rigidity: "9",
        hit: "3",
        guard: "1",
        damage: "20",
        stan: "70",
        remarks: "Can be rapid canceled",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching MP",
        outbreak: "6",
        persistence: "4",
        rigidity: "13",
        hit: "5",
        guard: "2",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching HP",
        outbreak: "8",
        persistence: "5",
        rigidity: "23",
        hit: "-2",
        guard: "-5",
        damage: "90",
        stan: "150",
        remarks: "Forces stand",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching LK",
        outbreak: "4",
        persistence: "2",
        rigidity: "8",
        hit: "3",
        guard: "0",
        damage: "20",
        stan: "70",
        remarks: "VS*Can only be canceled V-Skill II",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching MK",
        outbreak: "6",
        persistence: "2",
        rigidity: "14",
        hit: "2",
        guard: "-3",
        damage: "50",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Crouching HK",
        outbreak: "8",
        persistence: "2",
        rigidity: "25",
        hit: "D",
        guard: "-13",
        damage: "90",
        stan: "150",
        remarks: "Triggers Crush Counter  (D)\n                                                                                                                                                                VS*Can only be canceled V-Skill II",
        vtrigger: 2,
        type: "normal",
        command: ""
    },
    {
        name: "Jumping LP",
        outbreak: "4",
        persistence: "6",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "40",
        stan: "70",
        remarks: "",
        vtrigger: 2,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping MP",
        outbreak: "6",
        persistence: "7",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "60",
        stan: "100",
        remarks: "Causes blowback knockdown on airborne hit",
        vtrigger: 2,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping HP",
        outbreak: "8",
        persistence: "5",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "90",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping LK",
        outbreak: "4",
        persistence: "8",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "40",
        stan: "70",
        remarks: "Can cross-up",
        vtrigger: 2,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping MK",
        outbreak: "7",
        persistence: "5",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "jump",
        command: ""
    },
    {
        name: "Jumping HK",
        outbreak: "10",
        persistence: "5",
        rigidity: "",
        hit: "",
        guard: "",
        damage: "90",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "jump",
        command: ""
    },
    {
        name: "Floral Kick",
        outbreak: "21",
        persistence: "2",
        rigidity: "23",
        hit: "1",
        guard: "-7",
        damage: "70",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "unique",
        command: "→ + MK"
    },
    {
        name: "Furiko Upper",
        outbreak: "7",
        persistence: "3",
        rigidity: "26",
        hit: "-4",
        guard: "-7",
        damage: "80",
        stan: "150",
        remarks: "Triggers Crush Counter  (D)",
        vtrigger: 2,
        type: "unique",
        command: "← + HP"
    },
    {
        name: "Chin Buster Kick",
        outbreak: "12",
        persistence: "3",
        rigidity: "20",
        hit: "4",
        guard: "2",
        damage: "80",
        stan: "150",
        remarks: "Forces stand\n                                                                                                                VS*Can only be canceled V-Skill II\nCauses blowback knockdown on airborne hit",
        vtrigger: 2,
        type: "unique",
        command: "↘ + HK"
    },
    {
        name: "Hanakaze",
        outbreak: "5",
        persistence: "3",
        rigidity: "17",
        hit: "",
        guard: "",
        damage: "130",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "throw",
        command: "(NEAR OPPONENT) → OR + LP LK"
    },
    {
        name: "Sakura Shoulder Throw",
        outbreak: "5",
        persistence: "3",
        rigidity: "17",
        hit: "",
        guard: "",
        damage: "150",
        stan: "200",
        remarks: "",
        vtrigger: 2,
        type: "throw",
        command: "(NEAR OPPONENT) ← + LP LK"
    },
    {
        name: "Sailor Hop",
        outbreak: "5",
        persistence: "2",
        rigidity: "6 frame(s) after landing",
        hit: "",
        guard: "",
        damage: "160",
        stan: "200",
        remarks: "",
        vtrigger: 2,
        type: "throw",
        command: "(CLOSE TO OPPONENT IN AIR) LP LK"
    },
    {
        name: "[VS1] Harukaze",
        outbreak: "",
        persistence: "",
        rigidity: "54+15 frame(s) after landing",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "Follow-up attack timing differs when performed from a V-Trigger 2 powered Shunpukyaku",
        vtrigger: 2,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK"
    },
    {
        name: "[VS1] Sakura Otoshi",
        outbreak: "11",
        persistence: "3",
        rigidity: "6 frame(s) after landing(*115)",
        hit: "D",
        guard: "-2",
        damage: "50 (*2100) (*3150)",
        stan: "40 (*2120) (*3240)",
        remarks: "Can be performed up to 3 times on hit only. Startup for the 2nd and 3rd hits is 5F\nValue upon making contact with a standing Ryu, blocking, at the highest point\n+6F on block at the lowest point\n-9F on block at the highest point\nLanding recovery for the 3rd attack, hit only, is 5F\n*1on hit\n*2Up to the 2nd hit\n*3Up to the 3rd hit",
        vtrigger: 2,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK ▶ P"
    },
    {
        name: "[VS1] Oukakyaku",
        outbreak: "18",
        persistence: "Until landing",
        rigidity: "11 frame(s) after landing",
        hit: "3",
        guard: "-2",
        damage: "80",
        stan: "100",
        remarks: "Value upon making contact with a standing Ryu at the highest point\nLowest point: hit +12F, block +7F\nHighest point: hit +0F, block -5F",
        vtrigger: 2,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK ▶ K"
    },
    {
        name: "[VS1] Kashinfu",
        outbreak: "8",
        persistence: "3",
        rigidity: "15 frame(s) after landing",
        hit: "D",
        guard: "",
        damage: "150",
        stan: "200",
        remarks: "Can only throw standing opponents",
        vtrigger: 2,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL I) MP MK ▶ LP LK"
    },
    {
        name: "[VS2] Haru Hayate",
        outbreak: "",
        persistence: "",
        rigidity: "34 total frames",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "",
        vtrigger: 2,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL II) MP MK"
    },
    {
        name: "[VS2] Haru Hayate (Attack)",
        outbreak: "26",
        persistence: "5",
        rigidity: "14+11 frame(s) after landing",
        hit: "D",
        guard: "-7",
        damage: "70",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "vsystem",
        command: "(WHEN SELECTING VSKILL II) MP MK (HOLD BUTTONS)"
    },
    {
        name: "Sakura Senpu",
        outbreak: "1",
        persistence: "",
        rigidity: "5",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "V-Gauge Timer + 3500F\n                                                                                                                                                                                While active, Shouoken and Shunpukyaku will be strengthened",
        vtrigger: 2,
        type: "vsystem",
        command: "HP HK"
    },
    {
        name: "Floral Spin",
        outbreak: "17",
        persistence: "2",
        rigidity: "24(*27)",
        hit: "D",
        guard: "-2",
        damage: "60",
        stan: "0",
        remarks: "1F - 30F  attack & projectile invincibility\n                                                Recover 200 stun upon activation\non hit",
        vtrigger: 2,
        type: "vsystem",
        command: "(DURING GUARD) → + LK"
    },
    {
        name: "V-Shift",
        outbreak: "1",
        persistence: "9",
        rigidity: "22",
        hit: "",
        guard: "",
        damage: "",
        stan: "",
        remarks: "1F - 16F  throw invincibility\n                                                Allows you to evade strikes and projectiles from frames 1-9 (Increases the V-Gauge by 150 when the evasion triggers)\nCounterable during recovery\n17 frame move that's fully invincible until the end of recovery when evading attacks.\nTransitions to V-Shift Break when evading attacks.",
        vtrigger: 2,
        type: "vsystem",
        command: "HP MK"
    },
    {
        name: "Floral Side-Kick",
        outbreak: "8",
        persistence: "3",
        rigidity: "23",
        hit: "D",
        guard: "-2",
        damage: "60",
        stan: "0",
        remarks: "1F - 10F  full invincibility",
        vtrigger: 2,
        type: "vsystem",
        command: "HP MK ▶ (AFTER EVADING WITH V-SHIFT) HP MK"
    },
    {
        name: "L Hadoken",
        outbreak: "15",
        persistence: "42",
        rigidity: "48 total frames",
        hit: "-2",
        guard: "-8",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + LP"
    },
    {
        name: "M Hadoken",
        outbreak: "15",
        persistence: "49",
        rigidity: "48 total frames",
        hit: "-2",
        guard: "-8",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + MP"
    },
    {
        name: "H Hadoken",
        outbreak: "15",
        persistence: "56",
        rigidity: "48 total frames",
        hit: "-2",
        guard: "-8",
        damage: "60",
        stan: "100",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + HP"
    },
    {
        name: "Hadoken (Charge)",
        outbreak: "41",
        persistence: "",
        rigidity: "71 total frames",
        hit: "D",
        guard: "3",
        damage: "80",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + P (HOLD BUTTONS)"
    },
    {
        name: "EX Hadoken",
        outbreak: "14",
        persistence: "",
        rigidity: "45 total frames",
        hit: "D",
        guard: "1",
        damage: "100",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + P P"
    },
    {
        name: "Tengyo Hadoken",
        outbreak: "17",
        persistence: "27",
        rigidity: "45 total frames",
        hit: "D",
        guard: "4",
        damage: "60",
        stan: "100",
        remarks: "Will not hit crouching opponents",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + K"
    },
    {
        name: "Tengyo Hadoken (Charge)",
        outbreak: "39",
        persistence: "34",
        rigidity: "75 total frames",
        hit: "D",
        guard: "1",
        damage: "100",
        stan: "150",
        remarks: "Will not hit crouching opponents",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + K (HOLD BUTTONS)"
    },
    {
        name: "EX Tengyo Hadoken",
        outbreak: "15",
        persistence: "34",
        rigidity: "45 total frames",
        hit: "D",
        guard: "-1",
        damage: "100",
        stan: "150",
        remarks: "Will not hit crouching opponents",
        vtrigger: 2,
        type: "special",
        command: "↓↘→ + K K"
    },
    {
        name: "L Shouoken",
        outbreak: "4",
        persistence: "7",
        rigidity: "26+18 frame(s) after landing",
        hit: "D",
        guard: "-31",
        damage: "100",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "→↓↘ + LP"
    },
    {
        name: "M Shouoken",
        outbreak: "9",
        persistence: "7",
        rigidity: "28+18 frame(s) after landing",
        hit: "D",
        guard: "-32",
        damage: "110",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "→↓↘ + MP"
    },
    {
        name: "H Shouoken",
        outbreak: "14",
        persistence: "12",
        rigidity: "34+18 frame(s) after landing",
        hit: "D",
        guard: "-37",
        damage: "120",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "→↓↘ + HP"
    },
    {
        name: "VShouoken",
        outbreak: "4",
        persistence: "22",
        rigidity: "26+25 frame(s) after landing",
        hit: "D",
        guard: "-33",
        damage: "170",
        stan: "200",
        remarks: "Uses 1000F V-Gauge Timer\n                                                                                                                                1F - 12F  full invincibility\n                                                Subject to counter hit during move duration (1.2x damage)",
        vtrigger: 2,
        type: "special",
        command: "(DURING V-TRIGGER II) →↓↘ P"
    },
    {
        name: "EX Shouoken",
        outbreak: "4",
        persistence: "18",
        rigidity: "36+21 frame(s) after landing",
        hit: "D",
        guard: "-45",
        damage: "150",
        stan: "200",
        remarks: "1F - 12F  full invincibility\n                                                Subject to counter hit during move duration (1.2x damage)",
        vtrigger: 2,
        type: "special",
        command: "→↓↘ P P"
    },
    {
        name: "L Shunpukyaku",
        outbreak: "17",
        persistence: "2",
        rigidity: "13+9 frame(s) after landing",
        hit: "2",
        guard: "-2",
        damage: "60",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↙← + LK"
    },
    {
        name: "M Shunpukyaku",
        outbreak: "21",
        persistence: "4",
        rigidity: "10+14 frame(s) after landing",
        hit: "3",
        guard: "-6",
        damage: "80",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↙← + MK"
    },
    {
        name: "H Shunpukyaku",
        outbreak: "24",
        persistence: "6",
        rigidity: "8+14 frame(s) after landing",
        hit: "2",
        guard: "-2",
        damage: "100",
        stan: "150",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "↓↙← + HK"
    },
    {
        name: "VShunpukyaku (Swish or Guard)",
        outbreak: "12",
        persistence: "6",
        rigidity: "4+22 frame(s) after landing",
        hit: "",
        guard: "-10",
        damage: "",
        stan: "",
        remarks: "Uses 1000F V-Gauge Timer",
        vtrigger: 2,
        type: "special",
        command: "(DURING V-TRIGGER II) ↓↙← + K K"
    },
    {
        name: "VShunpukyaku (Hit)",
        outbreak: "12",
        persistence: "11",
        rigidity: "29",
        hit: "D",
        guard: "",
        damage: "120",
        stan: "150",
        remarks: "Uses 1000F V-Gauge Timer\n                                                                                                                                                                                VS*Only possible for the final part",
        vtrigger: 2,
        type: "special",
        command: "(DURING V-TRIGGER II) ↓↙← + K K"
    },
    {
        name: "EX Shunpukyaku",
        outbreak: "15(*12)",
        persistence: "6",
        rigidity: "4+18 frame(s) after landing",
        hit: "",
        guard: "-2",
        damage: "",
        stan: "",
        remarks: "*When canceled by standing light punch, crouching light punch, crouching medium kick\nV*Third attack can be canceled when blocked",
        vtrigger: 2,
        type: "special",
        command: "↓↙← + K K"
    },
    {
        name: "EX Shunpukyaku (Hit)",
        outbreak: "15",
        persistence: "11",
        rigidity: "23",
        hit: "D",
        guard: "",
        damage: "120",
        stan: "150",
        remarks: "V*Only possible during 3rd attack",
        vtrigger: 2,
        type: "special",
        command: "↓↙← + K K"
    },
    {
        name: "Airborne Shunpukyaku",
        outbreak: "13",
        persistence: "4",
        rigidity: "16 frame(s) after landing",
        hit: "D",
        guard: "",
        damage: "100",
        stan: "150",
        remarks: "Strength changes travel distance",
        vtrigger: 2,
        type: "special",
        command: "(DURING JUMP) ↓↙← + K"
    },
    {
        name: "EX Airborne Shunpukyaku",
        outbreak: "12",
        persistence: "19",
        rigidity: "16 frame(s) after landing",
        hit: "D",
        guard: "",
        damage: "140",
        stan: "200",
        remarks: "",
        vtrigger: 2,
        type: "special",
        command: "(DURING JUMP) ↓↙← + K K"
    },
    {
        name: "Sakura Rain",
        outbreak: "1+5",
        persistence: "",
        rigidity: "77 total frames",
        hit: "D",
        guard: "-23",
        damage: "250（*330）",
        stan: "0",
        remarks: "1F - 7F  full invincibility\n                                                *During release hit",
        vtrigger: 2,
        type: "ca",
        command: "↓↘→ ↓↘→ + P"
    }
];
exports.sakura = sakura;
//# sourceMappingURL=sakura.js.map